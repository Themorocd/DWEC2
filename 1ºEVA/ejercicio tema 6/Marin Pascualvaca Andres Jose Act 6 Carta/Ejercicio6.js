window.onload = () => {
    ...
    const cambiocart = () => {//esto lo que hace es imprimir cada opcion seleccionada en un div que tengo en el html
    ...
    cambiocart();
    motivo.forEach(x => x.addEventListener('change', function () {//recorro cada opcion hasta que encuentre la seleccionada y la meto en cambiocart()
        motivotext = this.value;
        cambiocart();
    }));
    medalla.forEach(x => x.addEventListener('change', function () {
        medallatext = this.value;
        cambiocart();
    }));
    objetivo.forEach(x => x.addEventListener('change', function () {
        objetivotext = this.value;
        cambiocart();
    }));
    despedida.forEach(x => x.addEventListener('change', function () {
